...
namespace Data.EntityFramework.BLL
{
    public class TriglavBL : IDisposable
    {
        ...
        public List<Post> getAllClanciWithoutTHIS(int PID)
        {
            return  context.Post.Where(x => x.PostVrsta == 1 && x.id != PID).ToList();
        }

        public List<Post> getAllPitanjaWithoutTHIS(int PID)
        {
            return context.Post.Where(x => x.PostVrsta == 5 && x.id != PID).ToList();
        }
        ...
        public List<Post> getPreporucenaPitanjaKorisnika(Korisnik KOR)
        {
            var query = (
                from k in context.Korisnik
                join ul in context.User_Likes on k.id equals ul.UserId
                join p in context.Post on ul.PostId equals p.id
                where (p.PostVrsta == 5) && (ul.Rate > 3 && ul.isLiked == 1) && k.BedzID > 1 && p.BrojOmiljenih > 2
        ...
        //funkcija prema formuli fri =wb*r, racuna ponderisane ocjene i zapisuje ih u bazu podataka
        public void AgentCalculationRatings()
        {
            List<User_Likes> UL = new List<User_Likes>();
            UL = getUserLikesCorrectRatings();
            foreach (var item in UL) {
                item.RatePonder = item.Rate * getBedzPonder(item.UserId.Value);
                context.SaveChanges();                
            }           
        }

        // funkcija vraca listu objekata validnih ocijenjenih postova
        public List<User_Likes> getUserLikesCorrectRatings()
        {
            var query = (
                from ul in context.User_Likes
              join p in context.Post on ul.PostId equals p.id 
            where (p.PostVrsta == 1 || p.PostVrsta == 1) && (ul.Rate != null && ul.Rate != 0)
                    select ul).ToList();
            return( List<User_Likes>)query;
        }

        // funkcija vraća vrijednost pondera na osnovu korisnickog bedza
        public float getBedzPonder(int UID)
        {
            Korisnik k = getKorisnikByID(UID);
            switch (k.BedzID)
            {
                case 1: return 1.0f;
                case 2: return 0.92f;
